...
if errorlevel 1 (
  call :fail "Cannot locate repository root from script path."
)

call :check_file "docs\CHANGELOG.md" "repository marker docs\CHANGELOG.md"
call :check_file "apps\dsa-web\package.json" "repository marker apps\dsa-web\package.json"
call :pass "Repository markers found."

call :check_file "apps\dsa-web\mock-only-preview\index.html" "mock-only preview HTML entry"
call :check_file "apps\dsa-web\src\mocks\preview-entry\mockOnlyPreviewEntry.ts" "mock-only preview TypeScript entry"
call :pass "Mock-only preview entry files found."

call :check_no_env_file ".env"
for %%F in (.env.*) do if /I not "%%~nxF"==".env.example" if exist "%%F" call :fail "Environment file exists at repository root. Remove it before dry-run."
call :check_no_env_file "apps\dsa-web\.env"
for %%F in (apps\dsa-web\.env.*) do if /I not "%%~nxF"==".env.example" if exist "%%F" call :fail "Environment file exists under apps\dsa-web. Remove it before dry-run."
call :pass "No .env files detected. Contents were not read."

if not "%DRY_RUN_HOST%"=="127.0.0.1" (
  call :fail "DRY_RUN_HOST must be 127.0.0.1."
)
call :pass "Host policy is fixed to 127.0.0.1."

node --version >nul 2>nul
if errorlevel 1 call :fail "Node is not available. Install Node manually, then rerun."
for /f "delims=" %%V in ('node --version') do echo PASS Node version: %%V

npm --version >nul 2>nul
if errorlevel 1 call :fail "npm is not available. Install npm manually, then rerun."
for /f "delims=" %%V in ('npm --version') do echo PASS npm version: %%V

if not exist "apps\dsa-web\node_modules" (
  call :fail "apps\dsa-web\node_modules is missing. Install dependencies manually before dry-run."
)
call :pass "apps\dsa-web\node_modules exists."

pushd "apps\dsa-web" >nul 2>nul
if errorlevel 1 call :fail "Cannot enter apps\dsa-web."

echo Running mock-only preview-entry test...
npm run test -- tests/mocks/preview-entry/mockOnlyPreviewEntry.test.ts
if errorlevel 1 call :fail "mockOnlyPreviewEntry.test.ts failed."

echo Running mock-only network-boundary test...
npm run test -- tests/mocks/preview/mockOnlyPreviewNetworkBoundary.test.ts
if errorlevel 1 call :fail "mockOnlyPreviewNetworkBoundary.test.ts failed."

echo Running mock-only preview model test...
npm run test -- tests/mocks/preview/mockOnlyPreview.test.ts
if errorlevel 1 call :fail "mockOnlyPreview.test.ts failed."
...
:check_no_env_file
if exist "%~1" (
  call :fail "Environment file exists: %~1. Remove it before dry-run."
)
exit /b 0
