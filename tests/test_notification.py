...
"""
    ...
    def test_discord_summary_includes_recent_change_vertical_data(self) -> None:
        content = """# 🎯 大盘复盘
...
"""
        ...
        self.assertIn("📊 近几日变化", summary)
        self.assertIn("• 上涨占比", summary)
        self.assertIn("  今日：41.2%", summary)
        self.assertIn("  昨日：52.3%", summary)
        self.assertIn("  变化：-11.1 个百分点", summary)
        self.assertIn("  判断：市场宽度明显转弱", summary)
        self.assertIn("• 成交额", summary)
        self.assertIn("  判断：成交额明显放大，分歧加剧", summary)
        self.assertIn("• 涨跌停差", summary)
        self.assertNotIn("|", summary)

    def test_discord_summary_includes_recent_change_history_shortage(self) -> None:
        content = """### 📊 近 5 日盘面对比
...
"""
        ...
        self.assertIn("📊 近几日变化", summary)
        self.assertIn("历史样本不足，已开始记录，后续日报将自动形成趋势对比。", summary)
...
"""
    ...
    def test_discord_index_table_becomes_global_mobile_cards(self) -> None:
        content = """### 二、指数结构
| 指数 | 最新 | 涨跌幅 | 开盘 | 最高 | 最低 | 振幅 | 成交额(亿) |
|------|------|--------|------|------|------|------|-----------|
| 上证指数 | 4028.90 | 🔴 -2.03% | 4100 | 4110 | 4020 | 2.1% | 1000 |
| 恒生指数 | 24500.50 | 🟢 +1.25% | 24000 | 24600 | 23900 | 1.5% | 800 |
| 道琼斯指数 | 39500.10 | +0.50% | 39300 | 39600 | 39200 | 1.0% | 0 |
| 纳斯达克指数 | 18000.20 | -0.75% | 18100 | 18200 | 17900 | 1.6% | 0 |
| 标普500指数 | 5200.30 | +0.10% | 5190 | 5210 | 5180 | 0.6% | 0 |
| 日经225 | 39000.40 | +1.20% | 38500 | 39100 | 38400 | 1.8% | 0 |
| TOPIX | 2800.50 | -0.30% | 2810 | 2820 | 2790 | 1.1% | 0 |
| KOSPI | 3100.00 | ⚪ 0.00% | 3100 | 3110 | 3090 | 0.4% | 300 |
"""
...
|------|------|--------|------|------|------|------|-----------|----------|
| 恒生科技指数 | 5500.00 | -1.20% | 5600 | 5650 | 5480 | 3.0% | 0 | 2026-07-01 |
"""
...
def test_discord_runtime_info_footer(monkeypatch):
    from src.notification import append_runtime_info_footer

    monkeypatch.setenv("REQUEST_ID", "gh-1-1")
    monkeypatch.setenv("TRIGGER_SOURCE", "manual_action")
    monkeypatch.setenv("MODE", "full")
    monkeypatch.setenv("MODEL_PROFILE", "daily")

    content = append_runtime_info_footer("日报正文")

    assert "请求编号：gh-1-1" in content
