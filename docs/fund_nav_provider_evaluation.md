...
接入路线为 dry-run → local-only → real-provider minimal gated adapter；P5-R2 local-only 仅验证本地 fixture response 映射，不联网、不请求真实基金净值。

## P5-R3 接入路线补充

场外基金净值 provider 接入路线为 dry-run → local-only → real-provider minimal gated adapter → local manual smoke test。real-provider minimal gated adapter 默认关闭，必须显式开启 `network_enabled`、`provider_enabled` 和 `allow_real_request` 后才允许本地手动调用注入 fetcher；CI 不联网，不请求真实基金净值。

## P5-R 接入路线补充

场外基金净值 provider 接入路线保持分阶段推进：dry-run → local-only → real-provider minimal gated adapter → local manual smoke test。P5-R4 的 local manual smoke test 只用于本地手动验证，不在 CI 中请求真实基金净值。
