...
Electron 驱动只通过 `bootstrapDesktopMain({ loadMain: () => undefined })` 注册既有凭证 IPC，不加载正式 `main.js`，不启动 Python 后端、更新检查或正式桌面窗口。子进程要求标准 `LOCALAPPDATA` 与 `DSA_SETTINGS_CREDENTIAL_SMOKE_LOCALAPPDATA` 完全一致，并在不把覆盖后的 `LOCALAPPDATA` 误判为用户真实目录的前提下继续执行绝对路径、系统临时目录、smoke 前缀和防逃逸校验。初始导航、设置页导航、preload bridge、字段发现和页面自动化失败均映射为固定低敏 `errorCode`，且 Mock 后端的认证 / 首次设置状态响应遵循前端现有 API 契约。
