...
P5-R6 新增 `fund_nav_display_adapter`，用于把审计后的场外基金净值结果转换为默认脱敏的页面展示模型；本阶段不接真实页面、不接真实 provider。
...
P5-R1 新增 `src/fund_nav_dry_run_provider.py`，用于在真实接入前验证 provider 请求计划和安全边界；dry-run 不请求真实基金净值，不保存真实数据。

P5-R7 新增账户级场外基金净值 provider dry-run 编排层：只处理 holding / watching 的 CN 场外 fund，stock / ETF / index 等资产继续走各自 quote 或指数模块；默认不联网、不请求真实基金净值。
