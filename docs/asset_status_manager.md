...
资产状态管理用于在账户 / 分组内表达资产从持有、收藏、清仓、归档到删除标记的状态变化。当前阶段只提供后端逻辑、预览结果、确认保护、结构化执行结果、文档和测试，不做真实网页页面，不读取真实用户配置，不接入日报 / 周报 / Discord，也不保存真实金额、成本价或账户资产。持有 vs 收藏对比分析见 `docs/holding_watch_compare.md`。
