...
Web-P32 是“股票基金质量分析系统”的 mock 报告 schema 文档阶段，只描述 Web-P31 已统一的 mock-only 日报 fixture 结构与页面字段契约。当前日报 / 推送显示名称固定为“AI股票基金每日信息报告”。
...
- 不发送日报、提醒或任何通知。
- 不执行交易。
- 不读取 `.env`、token、webhook、API key 或任何密钥配置。
...
| `redactionLabels` | `readonly string[]` | 是 | `REDACTED FIXTURE DATA`、`不读取数据库`、`不读取 .env` | 展示脱敏与不读取敏感来源的声明。 | 必须覆盖不读取 `.env`、token、webhook、API key、数据库、真实历史日报文件等语义。 |
...
```json
{
  ...
  "redactionLabels": ["REDACTED FIXTURE DATA", "不读取 .env", "不读取 token", "不读取 webhook", "不读取 API key", "不读取数据库"],
  "mockOnlyNotes": ["当前仍保持 mock-only，不接真实 API、provider、AI、通知、账户、数据库或交易。"]
}
```

该示例不包含真实基金代码、真实金额、真实通知目标、token、webhook、API key 或外部 URL。

## 10. 与后续 Web-P33 / Web-P34 的关系

- Web-P33 可以考虑本地 mock 数据目录规划，继续整理静态脱敏 fixture 的文件边界、命名、复用与测试保护。
- Web-P34 可以考虑前端预览字段与未来真实日报字段对齐，重点是字段语义、兼容层、脱敏策略和迁移步骤。
- Web-P33 / Web-P34 仍然不得接真实 API、provider、AI、通知、账户、数据库或交易，除非后续任务明确改变阶段边界并完成高规格复核与用户确认。
