...
7. 如果第 6 步选择安装/更新依赖，脚本会继续安装/更新 `pytest`，用于后续本地 smoke 测试。
8. 运行 `python main.py --help`。
9. 运行 `python main.py --local-smoke`。
10. 运行 `python -m pytest tests/test_windows_local_smoke.py -q`。
11. 运行 `python -m py_compile main.py src/notification.py tests/test_windows_local_smoke.py`。
12. 输出总体结果并保留窗口，方便截图。
...
- 输入 `Y`：先执行 `.venv\Scripts\python.exe -m pip install -r requirements.txt`，再执行 `.venv\Scripts\python.exe -m pip install pytest`。`pytest` 是运行 `tests/test_windows_local_smoke.py` 所需的本地验证依赖，脚本会自动处理。
- 输入其他内容：跳过安装，但继续后续验证；如果 `.venv` 中尚未安装 `pytest`，后续 pytest 检查会失败，最终结果会提示依赖可能未安装或未更新。
