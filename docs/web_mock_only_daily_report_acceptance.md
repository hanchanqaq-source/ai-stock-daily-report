...
| Web-P36 | 汇总当前链路、核心文件、安全边界、Windows 本地验收命令和 PR 复核清单。 | 本文档完成；仍不进入真实日报接入。 |
| Web-P37 | 补充 mock-only 页面预览入口和本地验收脚本说明。 | 已由 `docs/web_mock_only_preview_entry_local_checks.md` 承接；仍只允许 mock-only 本地安全预览。 |
...
Web-P37 已新增 `docs/web_mock_only_preview_entry_local_checks.md`，用于说明 mock-only 页面入口 `apps/dsa-web/mock-only-preview/index.html`、入口脚本 `apps/dsa-web/src/mocks/preview-entry/mockOnlyPreviewEntry.ts`、Windows 本地验收命令、人工预览只允许绑定 `127.0.0.1`，以及不得读取 `.env` / token / webhook / API 凭据、不得启动后端、不得连接真实 API、不得接 provider / AI / Agent / 通知 / 账户 / 数据库 / 交易的安全边界。

Web-P37 仍不改变页面视觉设计，不改变 TypeScript 运行逻辑，不修改 fixture、adapter、guard，不新增真实日报 JSON 示例，不自动启动 dev server，不自动打开浏览器。

## 9. 后续建议

- Web-P38 以后再考虑真实日报接入前的 schema 高规格复核，包括字段语义、兼容性、错误态、脱敏、provider 授权、凭证管理、通知 dry-run、账户授权、日志脱敏和回滚方案；但不能在 Web-P36 / Web-P37 阶段接入真实数据。
