...
真实场外基金净值 provider 结果进入页面前必须先通过 `fund_nav_result_audit`，再通过 `fund_nav_display_adapter` 生成默认脱敏的页面展示模型；审计失败、被阻断或发现 secret 时页面不显示真实基金净值。

统一汇总进入账户页面模型前还必须通过 P5-T `account_real_data_final_gate` 最终安全闸门；该闸门再次检查 audit、display adapter、redacted、repository safety、secret、真实金额、场外基金措辞和股票 / ETF 与基金字段边界。
...
## 场外基金净值本地手动 smoke 安全边界

`scripts/run_fund_nav_provider_smoke.py` 默认 dry-run，不联网、不读取真实 `user_config`、不保存真实净值或估算净值。真实请求必须本地手动显式启用，并在 CI / GitHub Actions 中被禁止。输出必须保持脱敏，真实结果只打印到控制台，且 `allow_commit_to_repo=false`。
