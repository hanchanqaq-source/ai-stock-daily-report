...
## [Unreleased]
- [新功能] 增加“明确授权后自动发布”工作流：仅接受仓库所有者的专用 Release 请求或精确手动确认，从 `main` 构建全部桌面产物，全部成功后才创建新 tag/Release，并复核 Windows 安装包、Portable ZIP、SHA-256、更新元数据与 macOS DMG；普通合并不会触发发布，既有版本禁止覆盖。
- [修复] Windows Release 构建不再以中文进程名调用 `Get-Process`；避免 GitHub Windows runner 解析脚本时发生字符编码错误，恢复安装包、Portable ZIP 与 `.sha256` 的生成路径。
...
- [新功能] Build E8 将问答会话按当前用户及股票/基金中心分区保存和筛选，切换中心或用户不再自动带入另一领域的历史上下文。
...
- [新功能] Build E5 为本机股票/基金快速持仓增加按用户隔离的变更历史，记录新增、编辑、删除与回收站恢复；历史不保存截图、密钥、设置、日志、真实账户或交易流水。
...
- [新功能] Build D3 在基金中心接入 1–4 只基金的手动只读行业周期与经营生产力代理证据：按可核验行业展示 20/60 日趋势、成交额变化、上涨股票比例、相对沪深300、固定阈值周期阶段及营收/利润/ROE/经营现金流代理；证据日期、置信度和缺失维度明确展示，不接 Qlib 依赖、AI、账户、交易、通知、持久化或 Build D4 配置建议。
- [新功能] Build D2 在基金中心接入 1–4 只基金的手动只读对比和披露行业穿透，展示资料、净值日期、前十大持仓/集中度、基金披露行业/前三集中度及两两披露重合下限；保持逐次确认、localhost、内存态和账户/交易/通知/AI/持久化全部禁用，行业周期与配置建议仍分别等待 Build D3/D4。
- [新功能] Build D1 将 AKShare 基金概况、正式净值和最新季度披露持仓接入 Build C 契约，并在基金首页提供六位代码、逐次确认、本机限定的手动只读查询；结果仅保存在页面内存，行业映射保持未知等待 Build D2，不接账户、交易、通知、AI 或持久化。
- [新功能] Build C 建立基金资料、净值、披露持仓、行业映射及统一来源元数据契约，正式默认只返回可审计的未接入/缺失状态，测试 fixture 需显式测试门禁才能使用；当前不接真实 Provider、分析建议或持久化。
...
- [改进] Web-P40 新增 dry-run schema TypeScript 类型草案，使用字面量类型锁定 dry-run、安全禁用开关和 mock-only 回退，并更新 mock-only boundary 测试扫描范围；当前仍不接真实 API / provider / AI / 通知 / 账户 / 数据库 / 交易。
...
- [改进] Web-P29 优化 mock-only 预览页移动端与窄屏响应式布局，页面总览、快速导航、模块卡片和返回入口在平板 / 手机宽度下自动换行或单列展示，并补充文档与静态测试。
- [改进] Web-P28 新增 mock-only 页面总览与模块完成度状态栏，从现有 sections 状态派生可预览 / 后续建设 / 总模块数量，展示 REDACTED FIXTURE DATA、127.0.0.1 only 和网络、通知、交易、模型调用安全边界，并补充文档与静态测试。
...
- [修复] L2O-fix-3 将 Windows localhost-only safe preview start 脚本改为直接通过 node 执行本地 Vite JS 入口，并在启动前 fail-fast 检查 node_modules\vite\bin\vite.js，避免 npm/Node 包装层弹出额外交互窗口。
...
- [改进] AI 股票基金每日盯盘报告 workflow 启用同分支并发取消、pip 缓存、45 分钟主任务超时和 PERF 配置日志，减少重复手动触发排队与依赖安装耗时。
- [改进] Discord 大盘复盘在盘面快照后新增“涨跌结构”，展示上涨/下跌集中板块与领涨/领跌个股 Top 5。
