...
- [新功能] Build D4 在基金中心接入当前用户基金组合风险与配置复核：浏览器只提交六位基金代码、归一化占比和可选目标仓位，后端复用 D2/D3 公开证据并补充正式净值 20/60 日收益、年化波动和最大回撤，按基金集中度、披露重合、行业集中、净值风险及周期压力输出固定阈值复核项；用户名、金额、成本和账户不进入请求，切换用户或修改持仓即清空结果，不接 AI、交易、通知、持久化或 Release。
