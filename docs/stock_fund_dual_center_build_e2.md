...
- 正式表：`workspace_users`、`workspace_portfolio_preferences`、`workspace_stock_holdings`、`workspace_fund_holdings`、`workspace_fund_watchlist_items`、`workspace_portfolio_backups`、`workspace_holding_recycle_entries`、`workspace_holding_history_entries`。
...
原有版本化 JSON 导出、导入预览、明确确认覆盖和恢复点能力继续保留。持久化阶段 E3 后，白名单只处理工作台用户、快速持仓与基金自选，不导出 `.env`、密钥、DPAPI 凭证、日志、数据库文件、真实账户或交易记录；旧版备份没有基金自选字段时按空列表兼容。
