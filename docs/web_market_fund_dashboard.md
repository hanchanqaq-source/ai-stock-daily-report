...
Web-P16 已增强 `sections.observation_points` 卡片展示，页面在同一个 demo `final_page_payload` 中渲染个人观察标签、状态 badge、风险等级 badge、脱敏观察点位、数据状态和“仅作为个人观察和记录，需用户自行判断。”免责声明。该增强不请求原始行情字段、不请求真实基金净值，不直接请求原始基金净值字段，也不直接消费 provider 原始数据。
