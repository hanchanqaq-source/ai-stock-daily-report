...
const failure = (status: Exclude<ProviderReadonlyPortResult['status'], 'candidate'>): ProviderReadonlyPortResult => ({ status, providerLabel: 'REDACTED_PROVIDER_LABEL', readOnly: true, redacted: true, errors: [`provider.${status}`], warnings: [], fallbackMode: 'mock-only', canFallbackToMockOnly: true })
const candidate = (payload = MOCK_ONLY_PROVIDER_CANDIDATE_PAYLOAD_FIXTURE): ProviderReadonlyPortResult => ({ status: 'candidate', providerLabel: 'REDACTED_PROVIDER_LABEL', readOnly: true, redacted: true, candidate: payload, errors: [], warnings: [] })
const port = (result: ProviderReadonlyPortResult, spy = vi.fn()): ProviderReadonlyPort => ({ mode: 'local-dry-run', providerLabel: 'REDACTED_PROVIDER_LABEL', networkEnabled: false, credentialReadEnabled: false, accountReadEnabled: false, readCandidate: spy.mockResolvedValue(result) })
...
describe('Core-M2 readonly provider framework chain review', () => {
  it('matches the flag/provider outcome matrix', async () => {
    const rows = [
      ...
      [{ enabled: true }, candidate({ ...MOCK_ONLY_PROVIDER_CANDIDATE_PAYLOAD_FIXTURE, candidateId: 'REAL_ID' }), 'blocked', false, false],
    ] as const
    for (const [featureFlag, providerResult, status, fallbackUsed, hasNormalized] of rows) {
      const result = await runProviderReadonlyDryRunPipeline({ featureFlag, provider: providerResult ? port(providerResult) : undefined })
      expect(result.status).toBe(status)
      expect(result.fallbackUsed).toBe(fallbackUsed)
      expect('normalizedInput' in result).toBe(hasNormalized)
    }
    const exception = await runProviderReadonlyDryRunPipeline({ featureFlag: { enabled: true }, provider: { ...port(failure('unavailable')), readCandidate: vi.fn().mockRejectedValue(new Error('SECRET')) } })
  ...
  it('keeps successful Provider Port to RealDailyReportDryRunInput chain mock-only and locked', async () => {
    const result = await runProviderReadonlyDryRunPipeline({ featureFlag: { enabled: true }, provider: port(candidate()) })
    ...
    expect(result.normalizedInput.source).toMatchObject({ sourceType: 'mock-only', providerName: 'REDACTED_PROVIDER_LABEL', isRealReadOnly: false })
    expect(result).toMatchObject({ allowRealProvider: false, allowRealAccountRead: false, allowNotificationSend: false, allowTrading: false, allowAiCall: false, requiresHumanApproval: true, fallbackMode: 'mock-only' })
  })
})
