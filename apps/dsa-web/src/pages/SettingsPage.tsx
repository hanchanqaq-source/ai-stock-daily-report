...
type RawPortableUpdateDownloadResult = { canceled?: unknown; downloaded?: unknown; releaseName?: unknown; error?: unknown };
...
type AppM41SettingsWorkspaceTab = 'credentials' | 'sources' | 'connection';

const APP_M41_SETTINGS_TABS: Array<{
  id: AppM41SettingsWorkspaceTab;
  label: string;
  description: string;
}> = [
...
const AppM41SettingsWorkspace: React.FC = () => {
  const [activeTab, setActiveTab] = useState<AppM41SettingsWorkspaceTab>('credentials');

  return (
    <SettingsSectionCard
      title="App-M4.1 设置工作台骨架"
      description="静态工作台入口；不读取 .env、不保存密钥、不持久化配置、不发起网络请求，既有设置功能继续保留。"
    >
      <div className="space-y-4" data-testid="app-m41-settings-workspace">
        <div className="grid gap-2 md:grid-cols-3" role="tablist" aria-label="App-M4.1 设置工作台">
          {APP_M41_SETTINGS_TABS.map((tab) => (
            <button
              key={tab.id}
              type="button"
              role="tab"
              ...
              className={`rounded-2xl border px-4 py-3 text-left text-sm transition ${
                activeTab === tab.id
                  ? 'border-[var(--settings-border-strong)] bg-[var(--settings-surface-hover)] text-foreground shadow-soft-card'
                  : 'settings-border bg-background/35 text-secondary-text hover:bg-[var(--settings-surface-hover)] hover:text-foreground'
              }`}
              onClick={() => setActiveTab(tab.id)}
            >
              <span className="block font-semibold">{tab.label}</span>
              <span className="mt-1 block text-xs leading-5 text-muted-text">{tab.description}</span>
            </button>
          ))}
        </div>
        ...
        {activeTab === 'sources' ? (
          <div className="space-y-3 rounded-2xl border settings-border bg-background/35 px-4 py-4">
            <div className="flex flex-wrap items-center justify-between gap-2">
              ...
              <span className="rounded-full border settings-border bg-card/70 px-3 py-1 text-xs font-semibold text-muted-text">默认关闭</span>
            </div>
            <dl className="grid gap-3 text-sm sm:grid-cols-2 lg:grid-cols-3">
              {[
                ['类型', '公开只读行情'],
                ['当前基础', '已具备手动 dry-run 基础'],
                ['默认状态', '默认关闭'],
                ['正式页面接入', '尚未启用'],
                ['账户读取', '无账户读取'],
                ['交易权限', '无交易权限'],
        ...
        {activeTab === 'connection' ? (
          <div className="rounded-2xl border settings-border bg-background/35 px-4 py-4">
            <h3 className="text-base font-semibold text-foreground">连接测试</h3>
            <p className="mt-2 text-sm leading-6 text-secondary-text">
              App-M4.1 尚未启用真实连接测试。
            </p>
...
const SettingsPage: React.FC = () => {
  ...
  const handlePortableUpdateDownload = async () => {
    if (!desktopRuntimeApi?.downloadPortableUpdate) return;
