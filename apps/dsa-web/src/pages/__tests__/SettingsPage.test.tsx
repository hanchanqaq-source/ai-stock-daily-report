...
describe('SettingsPage', () => {
  ...
  it('adds the App-M4.1 static workspace without replacing existing settings behavior', async () => {
    const fetchSpy = vi.spyOn(globalThis, 'fetch');
    const storageSetSpy = vi.spyOn(Storage.prototype, 'setItem');
    const indexedDbOpenSpy = window.indexedDB ? vi.spyOn(window.indexedDB, 'open') : null;

    render(<SettingsPage />);

    expect(await screen.findByRole('heading', { name: '系统设置' })).toBeInTheDocument();
    expect(screen.getByText('认证与登录保护')).toBeInTheDocument();
    expect(screen.getByTestId('app-m41-settings-workspace')).toBeInTheDocument();
    expect(screen.getByRole('heading', { name: 'App-M4.1 设置工作台骨架' })).toBeInTheDocument();
    expect(screen.getByRole('tab', { name: /接口与密钥/ })).toHaveAttribute('aria-selected', 'true');
    expect(screen.getByText(/不读取 \.env、不保存密钥、不持久化配置、不发起网络请求/)).toBeInTheDocument();
    expect(screen.getByText('AI 模型接口')).toBeInTheDocument();

    fireEvent.click(screen.getByRole('tab', { name: /数据源管理/ }));

    expect(screen.getByRole('tab', { name: /数据源管理/ })).toHaveAttribute('aria-selected', 'true');
    expect(screen.getByText('AkShare 公开 A 股行情')).toBeInTheDocument();
    expect(screen.getAllByText('公开只读行情')).toHaveLength(1);
    expect(screen.getAllByText('默认关闭')).toHaveLength(2);
    expect(screen.getByText('已具备手动 dry-run 基础')).toBeInTheDocument();
    ...
    expect(screen.getByRole('tab', { name: /连接测试/ })).toHaveAttribute('aria-selected', 'true');
    expect(screen.getByText('App-M4.1 尚未启用真实连接测试。')).toBeInTheDocument();
    expect(screen.getByRole('button', { name: '连接测试未启用' })).toBeDisabled();
    expect(screen.getByTestId('app-m41-settings-workspace')).not.toHaveTextContent(/连接成功|测试成功|延迟 \d+ms/);
