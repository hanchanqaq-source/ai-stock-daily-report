...
const hasNoteFragment = (notes: readonly string[], fragment: string): boolean => notes.some((note) => note.includes(fragment))
const isAllowedMockValue = (value: string): boolean =>
  value === 'MOCK_AMOUNT' || value === 'MOCK_RATIO' || value === 'REDACTED_VALUE' || value === 'MOCK_METRIC' || value.startsWith('MOCK_')

const appendError = (errors: string[], code: string, path?: string): void => {
  errors.push(path ? `${code}:${path}` : code)
}

const externalUrlPattern = new RegExp(`(?:${['ht', 'tp'].join('')}s?:\\/\\/|${['local', 'host'].join('')}|${['127', '0', '0', '1'].join('\\.')}|${['0', '0', '0', '0'].join('\\.')})`, 'i')
const personalContactPattern = new RegExp('(?:[A-Z0-9._%+-]+\\s*@\\s*[A-Z0-9.-]+\\.[A-Z]{2,}|1[3-9]\\d{9})', 'i')
const secretMarkerPattern = new RegExp(['tok' + 'en', 'web' + 'hook', 'api[_ -]?key', 'authorization', 'pass' + 'word', 'sec' + 'ret', 'bear' + 'er'].join('|'), 'i')
const rawProviderFieldPattern = new RegExp(['raw', 'Response', 'endpoint', 'request', 'Url', 'response', 'Body', 'cookies', 'headers'].join('|'), 'i')
const preciseValuePattern = /(?:[$¥€]\s*\d+(?:,\d{3})*(?:\.\d{2,})?|\b\d+(?:,\d{3})+\.\d{2,}\b|\b\d+\.\d{2,}%\b)/
...
const scanString = (value: string, path: string, errors: string[]): void => {
  ...
  if (rawProviderFieldPattern.test(value)) appendError(errors, 'sensitive-pattern.raw-provider-field', path)
  if (preciseValuePattern.test(value)) appendError(errors, 'sensitive-pattern.precise-value', path)
  if (realisticCodePattern.test(value) && !value.startsWith('MOCK_')) appendError(errors, 'sensitive-pattern.realistic-code', path)
  if (affirmativeTradePattern.test(value)) appendError(errors, 'riskSignal.invalid', path)
}

const scanSensitiveStrings = (value: unknown, path: string, errors: string[]): void => {
  if (typeof value === 'string') {
    scanString(value, path, errors)
    return
  }
  if (Array.isArray(value)) {
    value.forEach((item, index) => scanSensitiveStrings(item, `${path}[${index}]`, errors))
    return
  }
  if (isRecord(value)) {
    for (const [key, item] of Object.entries(value)) scanSensitiveStrings(item, `${path}.${key}`, errors)
  }
}
...
const validateSection = (section: unknown, index: number, errors: string[]): void => {
  ...
  if (!isNonEmptyString(candidate.sectionId) || !isNonEmptyString(candidate.title) || !isNonEmptyString(candidate.summary) || !Array.isArray(candidate.notes)) {
    appendError(errors, 'section.invalid', `input.sections[${index}]`)
  }
}
...
const validateMetric = (metric: unknown, index: number, errors: string[]): void => {
  ...
  if (!isNonEmptyString(candidate.metricId) || !isNonEmptyString(candidate.label) || !isNonEmptyString(candidate.valueLabel) || !isNonEmptyString(candidate.unitLabel) || !Array.isArray(candidate.notes)) {
    appendError(errors, 'metric.invalid', `input.metrics[${index}]`)
    return
  }
  if (!isAllowedMockValue(candidate.valueLabel)) appendError(errors, 'metric.valueLabel.invalid', `input.metrics[${index}].valueLabel`)
}
...
const validateRiskSignal = (signal: unknown, index: number, errors: string[]): void => {
  ...
  if (!isNonEmptyString(candidate.signalId) || !isNonEmptyString(candidate.label) || !isNonEmptyString(candidate.levelLabel) || !isNonEmptyString(candidate.valueLabel) || !Array.isArray(candidate.notes)) {
    appendError(errors, 'riskSignal.invalid', `input.riskSignals[${index}]`)
    return
  }
  if (!isAllowedMockValue(candidate.levelLabel)) appendError(errors, 'riskSignal.levelLabel.invalid', `input.riskSignals[${index}].levelLabel`)
  if (!isAllowedMockValue(candidate.valueLabel)) appendError(errors, 'riskSignal.valueLabel.invalid', `input.riskSignals[${index}].valueLabel`)
}

export const validateProviderCandidatePayload = (input: ProviderCandidatePayload): ProviderCandidatePayloadValidationResult => {
  const errors: string[] = []
  ...
  if (!Array.isArray(candidate.riskSignals) || candidate.riskSignals.length === 0) appendError(errors, 'riskSignals.required')
  ...
  scanSensitiveStrings(input, 'input', errors)
